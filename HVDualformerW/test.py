...
# Extract the desired part
            ...
            hist_result,cha_hist,hist_feaure = net_h(hist)
            _, weights = net(img,cha_hist,hist_feaure)


            if multi_scale:
                img_1 = resize(img, size=(int(0.5 * img.shape[2]), int(0.5 * img.shape[3])), interpolation='bilinear', align_corners=True)
                hist_result1,cha_hist1,hist_feaure1 = net_h(hist05)
                _, weights_1 = net(img_1,cha_hist1,hist_feaure1)
                ...
                hist_result2,cha_hist2,hist_feaure2 = net_h(hist025)
                _, weights_2 = net(img_2,cha_hist2,hist_feaure2)
            ...
            #     # with open(os.path.join(args.outdir, f"{'5d16sample'}.txt"), "a") as f:
            #     #     f.write(sample_info+'\n')
            #     mse_lst.append(mse)
            #     mae_lst.append(mae)
            ...
            #     # with open(os.path.join(args.outdir, f"{'5d16'}.txt"), "a") as f:
            ...
            for i, fname in enumerate(filename):
                ...
                name = os.path.join(out_dir, os.path.basename(fname).split("_")[0] + '_WB.png') if args.dataset == "cubeplus" else \
                    os.path.join(out_dir, "_".join(os.path.basename(fname).split("_")[:2]) + 'dsftc_128_WB.jpg')
                print('name',name)
                result.save(name)
...
def get_args():
  ...
  """
    ...
    parser.add_argument('-wbs', '--wb-settings', dest='wb_settings', nargs='+', default=['D', 'S', 'T', 'F', 'C'])  # default=['D', 'S', 'T', 'F', 'C'])
    ...
    parser.add_argument('-ted', '--testing-dir', dest='tedir', default='./data/mixed/', help='Testing directory')
    parser.add_argument('-od', '--outdir', dest='outdir', default='./results/', help='Results directory')
    ...
    parser.add_argument('-mn', '--model-name', dest='model_name', type=str,
                        ...
                        help='Model name')
    parser.add_argument('-emb', '--embed_dim', dest='embed_dim',
                        type=int, default=12, help='embed_dim.')
    return parser.parse_args()


if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO, format='%(levelname)s: %(message)s')
    logging.info('Testing HVDualformerW WB correction')
    args = get_args()
    ...
    net = hvdualformerW.VisNet(device=device, inchnls=3 * len(args.wb_settings),em_dim=args.embed_dim,wbset_num=len(args.wb_settings))
    checkpoint_modelh = torch.load(r'D:\mywork3_w\checkpoints_model\paper5_30\paper\12d\64tdsfc\Histoformer_epoch_64_225.pth')
    net_h.load_state_dict(checkpoint_modelh['state_dict'])
    checkpoint_model = torch.load(r'D:\mywork3_w\checkpoints_model\paper5_30\paper\12d\64tdsfc\CAformer_epoch_64_225.pth')
    net.load_state_dict(checkpoint_model['state_dict'])
