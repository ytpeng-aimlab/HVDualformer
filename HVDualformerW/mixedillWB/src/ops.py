...
def histogram_loaderpatch(image):
    ...
    for i in range(b):
      imagep = image[i,:,:,:]
      R_hist, R_bins = np.histogram(imagep[:, :, 0], bins=256, range=(0, 256)) # R_hist.shape = (256,)
      G_hist, G_bins = np.histogram(imagep[:, :, 1], bins=256, range=(0, 256))
      B_hist, B_bins = np.histogram(imagep[:, :, 2], bins=256, range=(0, 256))

      R_pdf = R_hist/sum(R_hist)
      G_pdf = G_hist/sum(G_hist)
      B_pdf = B_hist/sum(B_hist)
      RGB.append(np.vstack((R_pdf, G_pdf, B_pdf)))

    RGB = np.stack(RGB, axis=0)
    tensor_RGB = torch.tensor(RGB, dtype=torch.float64)
    return tensor_RGB

def histogram_loadertest(image):
    image = (image*255).astype(np.uint8)
    h,w,c = image.shape
    ...
    tensor_RGB = torch.tensor(RGB, dtype=torch.float32)
