    ...
    """
        ...
        if self.mode == 'training':
            ...
            if self.shuffle_order:
                ...
                hists = torch.cat((d_hist,s_hist,t_hist), dim=1)

                if f_img is not None:
                    img = torch.cat((img, f_imgt), dim=1)
                    hists = torch.cat((hists, f_hist), dim=1)

                if c_img is not None:
                    img = torch.cat((img, c_imgt), dim=1)
                    hists = torch.cat((hists, c_hist), dim=1)

            return {'image': img, 'gt': gt_imgt, 'hist':hists, 'gthist':gt_hist,'filename': base_name}
