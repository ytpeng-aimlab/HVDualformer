...
parser.add_argument('--save_image_dir', type=str, default ='./results_5_30/cube257d8/',  help='save image dir')
...
model.to('cuda')    
weight_path = r'D:\my_work\checkpoints\115mywork_5_30\cubed8\Histoformer_3_epoch_207.pth'
weight_pathu = r'D:\my_work\checkpoints\115mywork_5_30\cubed8\CAformer_3_epoch_207.pth'
checkpoint = torch.load(weight_path)
checkpointu = torch.load(weight_pathu)
# # 讀取模型權重
model.load_state_dict(checkpoint['state_dict'])
# 步驟3: 使用model.load_state_dict()方法載入權重
...
with torch.no_grad():  #如果沒有這行，那下面在取值的時候要用.detach().numpy()
    for i,(inputs, ori_img,label_img,path) in enumerate(testloader):
        ...
        for gt_, out_ in zip(label_img.float().permute(0, 2, 3, 1), out_img.float().permute(0, 2, 3, 1)):
            ...
            with open(os.path.join(opt.outdir, f"{'HM30_cubesample207test'}.txt"), "a") as f:
               f.write(sample_info+'\n')
            mse_lst.append(mse)
            mae_lst.append(mae)
        ...
        with open(os.path.join(opt.outdir, f"{'HM30_cube207test'}.txt"), "w+") as f:
            f.write(final_info)
